...
import 'package:mathmentalist/src/data/models/math_grid.dart';
import 'package:mathmentalist/src/service/dialog_service.dart';
import 'package:mathmentalist/src/core/app_constant.dart';
import 'package:mathmentalist/src/data/repository/math_grid_repository.dart';
...
class MathGridProvider with ChangeNotifier {
  ...
  Future<void> checkResult(int index, MathGridCellModel gridModel) async {
    ...
    var listOfUnRemovedIndex = _currentState.listForSquare
        .where((result) => result.isRemoved == false)
        .toList();
    if (listOfUnRemovedIndex.length <= 12) {
      var possiblePairs = getCountOfPossiblePairs(
          listOfUnRemovedIndex, _currentState.currentAnswer);
      _index = _index + 1;
      _currentState = _list[_index];
    }
